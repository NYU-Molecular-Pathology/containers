...
# get script variable
input_vcf="${1:-xxxxxxxnonexxxxxxxx}"
sample_ID="${2:-xxxxxxxnonexxxxxxxx}"
avinput_file="${3:-${sample_ID}.avinput}"
annovar_output_file="${4:-${sample_ID}.${build_version}_multianno.txt}"

if [ "${input_vcf}" != "xxxxxxxnonexxxxxxxx" ]; then
    :
else
...
if [ "${sample_ID}" != "xxxxxxxnonexxxxxxxx" ]; then
    :
else
...
convert2annovar.pl --format vcf4old --includeinfo "${input_vcf}" --outfile "${avinput_file}"
